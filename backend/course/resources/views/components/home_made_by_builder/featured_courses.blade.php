                ...
                <div class="res-control d-flex align-items-center justify-content-between">
                    ...
                    <span class="featured-course-all-button">
                        <a href="{{ route('courses') }}" class="eBtn gradient builder-editable"
                            builder-identity="3">{{ get_phrase('View All Courses') }}</a>
                    </span>
                </div>
            </div>
        </div>
                ...
                @foreach ($featured_courses->take(4) as $key => $row)
                    <div class="col-lg-6 col-md-12 col-sm-6 mb-30">
                        <a href="{{ route('course.details', $row->slug) }}"
                            class="single-feature p-15 w-100 checkPropagation">
                            <div class="row">
                                        ...
                                        @auth
                                            @if ($row->wishlists()->where('course_id', $row->id)->count() > 0)
                                                <span data-bs-toggle="tooltip"
                                                    ...
                                                    class="heart checkPropagation inList"
                                                    onclick="wishlistToggle('{{ $row->id }}', this)"><i
                                                        class="fa-regular fa-heart"></i></span>
                                            @else
                                                <span data-bs-toggle="tooltip"
                                                    data-bs-title="{{ get_phrase('Add to wishlist') }}"
                                                    class="heart checkPropagation"
                                                    onclick="wishlistToggle('{{ $row->id }}', this)"><i
                                                        class="fa-regular fa-heart"></i></span>
                                            @endif
                                        @endauth
                                ...
                                <div class="col-lg-7 col-md-7">
                                    <div class="entry-details">
                                        ...
                                        <div class="creator mt-54">
                                            ...
                                            <p>{{ get_phrase('by') }}
                                                <span>{{ course_by_instructor($row->id)->name }}</span>
                                            </p>
                                        </div>
                                    </div>
                                </div>
