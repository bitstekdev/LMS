                            ...
                            <a href="#">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                    ...
                                    <path
                                        ...
                                        stroke="#192335" stroke-width="1.25" />
                                    <path
                        ...
                        <li class="primary-end">
                            <form action="{{ route('select.lng') }}" method="get">
                                <select name="language" id="lng-selector" class="form-select nice-control">
                                    @php
                                        $activated_language = strtolower(
                                            session('language') ?? get_settings('language'),
                                        );
                                    @endphp
                                    @foreach (App\Models\Language::all() as $lng)
                                        <option value="{{ $lng->name }}" @selected(strtolower($lng->name) == $activated_language)>
                                            {{ $lng->name }}</option>
                                    @endforeach
                                </select>
                            </form>
