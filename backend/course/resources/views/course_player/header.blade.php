            ...
            <div class="course-playing-header d-flex align-items-center justify-content-between py-1">
                ...
                <div class="playing-header-btns d-flex align-items-center">
                    ...
                    @if (is_course_instructor($course_details->id) || auth()->user()->role == 'admin')
                        <a href="{{ route(auth()->user()->role . '.course.edit', ['id' => $course_details->id, 'tab' => 'curriculum']) }}"
                            class="my-courses-btn d-none d-lg-block py-1 px-2">{{ get_phrase('Manage Course') }}</a>
                    @else
                        <a href="{{ route('my.courses') }}"
                            class="my-courses-btn d-none d-lg-block py-1 px-2">{{ get_phrase('My Courses') }}</a>
                    @endif
                </div>
            </div>
        </div>
