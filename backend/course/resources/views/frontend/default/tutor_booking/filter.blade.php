...
@endphp
<div class="offcanvas-lg offcanvas-start lms1-category-offcanvas" tabindex="-1" id="offcanvasSidebar"
                                ...
                                @foreach ($categories as $category)
                                    <li>
                                        <div class="form-check form-checkbox">
                                            <input class="form-check-input form-checkbox-input" type="radio"
                                                name="category" value="{{ $category->slug }}"
                                                id="category-{{ $category->id }}"
                                                @if (request()->has('category') && request()->input('category') == $category->slug) checked @endif>
                            ...
                            <ul class="d-flex flex-column gap-3">
                                @foreach ($subjects as $subject)
                                    <li>
                                        <div class="form-check form-checkbox">
                                            <input class="form-check-input form-checkbox-input" type="radio"
                                                name="subject" value="{{ $subject->slug }}"
                                                id="subject-{{ $subject->id }}"
                                                @if (request()->has('subject') && request()->input('subject') == $subject->slug) checked @endif>
                                            <label class="form-check-label form-checkbox-label"
                    ...
                    <li class="side-accordion-price py-3">
                        ...
                        <div>
                            ...
                            <div class="d-flex align-items-center gap-8 flex-wrap justify-content-between">
                                <p class="slider-range-value">{{ get_phrase('Min') }}: $<span
                                        class="from-slider-value2">{{ $lowestFee }}</span></p>
                                <p class="slider-range-value">{{ get_phrase('Max') }}: $<span
                                        class="to-slider-value2">{{ $highestFee }}</span></p>
                                <!-- Hidden inputs for min and max -->
                                <input type="text" class="from-slider-value form-control" name="min_fee"
                                    value="{{ $min_fee }}" hidden />
                                <input type="text" class="to-slider-value form-control" name="max_fee"
                                    value="{{ $max_fee }}" hidden />
                            </div>
                        </div>
                    </li>
                    <li class="side-accordion-item">
                                ...
                                @for ($i = 5; $i >= 1; $i--)
                                    <li>
                                        <input class="form-check-input" type="radio" name="rating"
                                            value="{{ $i }}" id="raging-{{ $i }}"
                                            @if (request()->has('rating') && request()->input('rating') == $i) checked @endif />
