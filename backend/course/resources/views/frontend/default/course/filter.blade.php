    ...
    <div class="row mb-4">
        ...
        <div class="col-6 text-end">
            @if (count(request()->all()) > 0 || !empty($category_details))
                <a class="btn d-flex align-items-center float-end border py-2" href="{{ route('courses') }}"><i
                        class="fi-rr-cross-circle me-2"></i> <span>{{ get_phrase('Clear') }} @if (isset($_GET) && count($_GET) > 0)
                            ({{ count($_GET) }})
                        @endif
                    </span></a>
            @endif
