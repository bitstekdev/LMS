@php
    ...
    if ($model === 'InstructorPayment') {
        ...
        $keys = $instructor->paymentkeys['maxicash'] ?? [];

        if ($payment_gateway->test_mode == 1) {
            $key_type = 'merchant_id';
            $key = $keys['merchant_id'] ?? '';
        } else {
            $key_type = 'merchant_password';
            $key = $keys['merchant_password'] ?? '';
        }
        ...
        $keys = $payment_gateway->keys['maxicash'] ?? ($payment_gateway->keys ?? []);
