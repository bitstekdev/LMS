                ...
                <div class="col-lg-7">
                    <div class="ol-card">
                        <form class="required-form"
                            action="{{ route('instructor.schedule_update', ['id' => $schedule_details->id]) }}"
                            ...
                            <div class="ol-card-body mb-20px p-20px">
                                ...
                                <div class="row">
                                            ...
                                            <select class="ol-select2" name="category_id" id="category_id" required>
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->category_id }}"
                                                        @if ($schedule_details->category_id == $category->category_id) selected @endif>
                                    ...
                                    @php
                                        $canTeach = App\Models\TutorCanTeach::where(
                                            'category_id',
                                            $schedule_details->category_id,
                                        )->get();
                                    @endphp
                                        ...
                                        <div class="w-100 mb-3 load_subjects">
                                            ...
                                            <select class="ol-select2" name="subject_id" id="subject_id" required>
                                                @foreach ($canTeach as $teach)
                                                    <option value="{{ $teach->subject_id }}"
                                                        @if ($schedule_details->subject_id == $teach->subject_id) selected @endif>
                                                        {{ $teach->category_to_tutorSubjects->name }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="paid-section" id="paid-section">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="datetime-local" class="form-control ol-form-control"
                                                ...
                                                value="{{ date('Y-m-d\TH:i', $schedule_details->start_time) }}" required>
                                        </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="ol-select2" id="duration" name="duration">
                                        ...
                                        @for ($i = 60; $i <= 300; $i += 15)
                                            <option value="{{ $i }}"
                                                {{ $schedule_details->duration == $i ? 'selected' : '' }}>
                                                {{ intdiv($i, 60) }}
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="description" placeholder="{{ get_phrase('Enter Description') }}"
                                        class="form-control ol-form-control text_editor">{{ $schedule_details->description }}</textarea>
                                </div>
