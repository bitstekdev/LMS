...
@endphp
<form class="ajaxFormSubmission" action="{{ route('instructor.my_subject_update', $mySubDetails->id) }}" method="post"
        ...
        <div class="col-12">
                ...
                <select class="form-control ol-form-control ol-select2" name="category_id" id="category_id" required>
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}"
                            {{ $mySubDetails->category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}
                        </option>
                    @endforeach
                </select>
            </div>
                ...
                <select class="form-control ol-form-control ol-select2" name="subject_id" id="subject_id" required>
                    @foreach ($subjects as $subject)
                        <option value="{{ $subject->id }}"
                            {{ $mySubDetails->subject_id == $subject->id ? 'selected' : '' }}>{{ $subject->name }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="price" class="form-control ol-form-control" id="price" min="1"
                    ...
                    value="{{ $mySubDetails->price }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="description" placeholder="{{ get_phrase('Enter Description') }}"
                    class="form-control ol-form-control text_editor" required>{{ $mySubDetails->description }}</textarea>
            </div>
