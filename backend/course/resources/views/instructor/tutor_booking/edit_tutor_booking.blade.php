...
@endphp
<form action="{{ route('instructor.booking_status_update', ['status' => 'update', 'id' => $id]) }}" method="post"
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control ol-form-control" id="title" name="title" placeholder="Topic title"
            value="{{ $booking_details->title }}" required>
    </div>
        ...
        <select class="ol-select2" name="category_id" id="category_id" required>
            ...
            @foreach ($categories as $category)
                <option value="{{ $category->id }}"
                    {{ $category->id == $booking_details->category_id ? 'selected' : '' }}>{{ $category->name }}
                </option>
            @endforeach
        </select>
    </div>
    ...
    <div class="mb-3 load_categories">
        ...
        <select class="ol-select2" id="sub_category_id" name="sub_category_id" required>
            @if ($booking_details->sub_category_id != 0)
                <option value="{{ $booking_details->sub_category_id }}">
                    {{ get_phrase($booking_details->sub_category->name) }}</option>
            @else
    ...
    <div class="mb-3">
        ...
        <div class="ol-radio-wrap d-flex flex-wrap">
            <div class="form-check form-check-radio">
                <input type="radio" class="form-check-input form-check-input-radio" id="online" name="class_type"
                    value="1" @if ($booking_details->tution_class_type == '1') checked @endif required>
                <label for="online" class="form-check-label">{{ get_phrase('Online') }}</label>
            </div>

            <div class="form-check form-check-radio">
                <input type="radio" class="form-check-input form-check-input-radio" id="in_person" name="class_type"
                    value="2" @if ($booking_details->tution_class_type == '2') checked @endif required>
    ...
    <div class="mb-3">
        ...
        <div class="ol-radio-wrap d-flex flex-wrap">
            <div class="form-check form-check-radio">
                <input type="radio" class="form-check-input form-check-input-radio" id="fixed" name="price_type"
                    value="fixed" @if ($booking_details->price_type == 'fixed') checked @endif required>
                <label for="fixed" class="form-check-label">{{ get_phrase('Fixed') }}</label>
            </div>

            <div class="form-check form-check-radio">
                <input type="radio" class="form-check-input form-check-input-radio" id="hourly" name="price_type"
                    value="hourly" @if ($booking_details->price_type == 'hourly') checked @endif required>
    ...
    <div class="mb-3">
        ...
        <input type="number" class="form-control ol-form-control" name="price" id="price" min="0"
            step="any" placeholder="Put a price" value="{{ $booking_details->price }}" required>
    </div>
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control ol-form-control" id="meeting_link" name="meeting_link"
            placeholder="zoom invitation link" value="{{ $booking_details->meeting_link }}">
    </div>
