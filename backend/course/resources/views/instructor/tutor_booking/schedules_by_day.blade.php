    ...
    <div class="ol-card radius-8px">
        <div class="ol-card-body my-3 py-12px px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                ...
                <a href="{{ route('instructor.manage_schedules') }}"
                                    ...
                                    <ul class="dropdown-list">
                                        <li>
                                            <a class="dropdown-item export-btn" href="#"
                                                onclick="downloadPDF('.print-table', 'schedule-list')"><i
                                            ...
                                            @foreach ($schedules as $key => $schedule)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="print-d-none">
                                                        <span
                                                            class="badge bg-{{ $status }}">{{ get_phrase(ucfirst($slot_status)) }}</span>
                                                    </td>
                                                            ...
                                                            <ul class="dropdown-menu">
                                                                <li>
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('instructor.schedule_edit', ['id' => $schedule->id]) }}">{{ get_phrase('Edit schedule') }}</a>
                                                                </li>
                                                                @if ($check == 0)
                                                                    <li>
                                                                        <a class="dropdown-item"
                                                                            onclick="confirmModal('{{ route('instructor.schedule_delete', $schedule->id) }}')"
                                                                            href="javascript:void(0)">{{ get_phrase('Delete schedule') }}</a>
                                                                    </li>
                                                                @endif
                                                            </ul>
                                                        </div>
