...
<form action="{{ route('instructor.bootcamp.module.update', $id) }}" method="post">@csrf
    ...
    <div class="fpb-7 mb-3">
        ...
        <select class="form-control ol-form-control ol-select2" data-toggle="select2" name="restriction">
            <option value="0" @if ($module->restriction == 0) selected @endif>
                {{ get_phrase('Select an option') }}</option>
            <option value="1" @if ($module->restriction == 1) selected @endif>
                {{ get_phrase('Until start date, keep this module locked.') }}</option>
            <option value="2" @if ($module->restriction == 2) selected @endif>
                {{ get_phrase('Keep this module open only within the selected date range.') }}</option>
        </select>
    </div>
    ...
    <div class="fpb7 mb-2">
        ...
        <input type="text" class="form-control ol-form-control daterangepicker"
            name="validity"value="{{ date('m/d/Y', $module->publish_date) . ' - ' . date('m/d/Y', $module->expiry_date) }}"
            required />
    </div>
