...
@if ($education) <!-- Check if education data exists -->
    <form class="ajaxFormSubmission" action="{{ route('instructor.manage.education_update', ['index' => $index]) }}"
        method="post" enctype="multipart/form-data">
        @csrf

        <div class="form-group mb-3">
            <label class="form-label ol-form-label">{{ get_phrase('Title') }}</label>
            <input type="text" name="title" class="form-control ol-form-control" value="{{ $education['title'] }}"
                required>
        </div>
        ...
        <div class="form-group mb-3">
            <input type="checkbox" name="status" id="status" value="ongoing" class="form-check-input"
                @if ($education['status'] === 'ongoing') checked @endif>
            <label for="status"
        ...
        <div class="text-center">
            <button class="btn ol-btn-primary ol-btn-sm w-100 formSubmissionBtn" type="submit"
                name="button">{{ get_phrase('Save') }}</button>
        </div>
    </form>
@else
