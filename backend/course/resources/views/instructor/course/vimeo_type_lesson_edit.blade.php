...
<div class="form-group mb-2">
    ...
    <input value="{{ $lessons->lesson_src }}" type="text" id="video_url" onblur="ajax_get_video_details(this.value)"
        name="lesson_src" class="form-control ol-form-control">
    <small class="form-label text-danger text-12px d-hidden mb-0" id="invalid_url">{{ get_phrase('Invalid url') }}.
        {{ get_phrase('Your video source has to be either Vimeo') }}</small>
</div>
...
<div class="form-group mb-2">
    ...
    <input default-seconds="{{ duration_to_seconds($lessons->duration) }}" type="text" name="duration" id="duration"
        class="form-control ol-form-control" readonly>
</div>
