@php
    $live_class = App\Models\LiveClass::where('id', $id)->first();
@endphp
<form class="required-form" action="{{ route('instructor.live.class.update', ['id' => $id]) }}" method="post">
    ...
    <div class="fpb-7 mb-3">
        ...
        <input type="text" value="{{ $live_class->class_topic }}" name="class_topic" id = "class_topic"
    ...
    <div class="fpb-7 mb-3">
        <label class="form-label ol-form-label" for="class_date_and_time">{{ get_phrase('Class date and time') }}<span
                class="required">*</span></label>
        <input value="{{ date('Y-m-d\TH:i', strtotime($live_class->class_date_and_time)) }}" type="datetime-local"
            class="form-control ol-form-control" name="class_date_and_time" id="class_date_and_time" required>
    </div>
