    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn ol-btn-secondary"
...
<script type="text/javascript">
    ...
    myModalEl.addEventListener('hidden.bs.modal', event => {
        $('#ajaxModal .modal-body').html(
            '<div class="py-5 text-center w-100"><div class="my-5 spinner-border" role="status"><span class="visually-hidden"></span></div></div>'
        );
    })
            ...
            complete: function(xhr) {
                ...
                if (/^[\],:{}\s]*$/.test(response.replace(/\\["\\\/bfnrtu]/g, '@').replace(
                    ...
                    for (let i = 0; i < parsedJson.length; i++) {
                        var exi = i + 1;
                        var img =
                            '<div class="p-2 w-50 position-relative"><a class="px-1 py-0 m-1 position-absolute btn btn-success" href="admin/ai_img_download?index=' +
                            exi +
                    ...
                    $('#aiResultHeader').html(
                        '<span class="text-14px">{{ get_phrase('Generated text') }}:</span> <a href="javascript:;" onclick="copy_text(this)" data-toggle="tooltip" data-placement="top" title="{{ get_phrase('Copy') }}" class="float-right p-0 btn"><i class="far fa-copy"></i> {{ get_phrase('Copy') }}</a>'
                    );
                }
