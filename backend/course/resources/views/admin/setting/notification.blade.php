                ...
                @foreach (json_decode($row->user_types, true) as $type)
                    <div class="col-auto">
                        ...
                        <div class="custom-control custom-switch mb-2">
                            <input type="checkbox" class="form-check-input" id="{{ $row->id . $type }}_system"
                                @if ($system_notification[$type]) checked @endif
                                @if ($row->is_editable != 1) disabled @endif>
                            <label
                                onclick="notification_enable_disable('{{ $row->id }}', '{{ $type }}', 'system')"
                                class="form-check-label"
                                for="{{ $row->id . $type }}_system">{{ get_phrase('System notification') }}
                            </label>
                        </div>

                        <div class="custom-control custom-switch mb-2">
                            <input type="checkbox" class="form-check-input" id="{{ $row->id . $type }}_email"
                                @if ($email_notification[$type]) checked @endif
                                @if ($row->is_editable != 1) disabled @endif>
                            <label
                                onclick="notification_enable_disable('{{ $row->id }}', '{{ $type }}', 'email')"
                                class="form-check-label"
                                for="{{ $row['id'] . $type }}_email">{{ get_phrase('Email notification') }}</label>
                        </div>
                    </div>
                @endforeach
            </div>
