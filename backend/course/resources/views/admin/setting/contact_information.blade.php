...
@php
    $contact_info = get_frontend_json_setting('contact_info', [
        'email' => '',
        'phone' => '',
        'address' => '',
        'office_hours' => '',
...
<form action="{{ route('admin.website.settings.update') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="col-md-7">
            ...
            <div class="fpb-7 mb-3">
                <label class="form-label ol-form-label">{{ get_phrase('Location') }} <small
                        class="text-12px text-muted">({{ get_phrase('Latitude') }},
                        {{ get_phrase('Longitude') }})</small></label>
                <input name="location" class="form-control ol-form-control" placeholder="40.689880, -74.045203"
