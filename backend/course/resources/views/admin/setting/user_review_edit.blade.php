    ...
    <div class="ol-card radius-8px">
        <div class="ol-card-body my-3 py-12px px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                ...
                <a href="{{ route('admin.website.settings', ['tab' => 'reviews']) }}"
        ...
        <div class="col-md-8">
            <div class="ol-card p-4">
                <div class="ol-card-body">
                    <form action="{{ route('admin.review.update', $review_data->id) }}" method="post"
                            ...
                            <select class="form-control ol-form-control ol-select2" data-toggle="select2" name="user_id"
                                ...
                                @foreach ($userList as $user)
                                    <option value="{{ $user->id }}"
                                        {{ $user->id == $review_data->user_id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="fpb-7 mb-3">
                            <label class="form-label ol-form-label"
                                for="summernote-basic">{{ get_phrase('Review') }}</label>
