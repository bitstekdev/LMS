                    ...
                    <div class="ol-card-body">
                        ...
                        <form action="{{ route('admin.drip.settings.update') }}" method="post">
                            ...
                            <div class="fpb-7 mb-3">
                                <label class="form-label ol-form-label">{{ get_phrase('Lesson completion role') }}<span
                                        class="required">*</span></label>
                                <br>
                                <input class="form-check-input" type="radio"
                                    onchange="$('.toggleMinimumWatchField').toggleClass('d-hidden');" value="percentage"
                                    id="video_percentage_wise" name="lesson_completion_role"
                                    @if ($drip_info->lesson_completion_role == 'percentage') checked @endif>
                                <label class="form-label ol-form-label"
                                    for="video_percentage_wise">{{ get_phrase('Video percentage wise') }}</label>
                                &nbsp;&nbsp;
                                <input class="form-check-input" type="radio"
                                    onchange="$('.toggleMinimumWatchField').toggleClass('d-hidden');" value="duration"
                                    id="video_duration_wise" name="lesson_completion_role"
                                    @if ($drip_info->lesson_completion_role == 'duration') checked @endif>
                                <label class="form-label ol-form-label"
                                    for="video_duration_wise">{{ get_phrase('Video duration wise') }}</label>
                            </div>

                            <div
                                class="fpb-7 mb-2 toggleMinimumWatchField @if ($drip_info->lesson_completion_role == 'percentage') d-hidden @endif  ">
                                <label class="form-label ol-form-label"
                                    for="minimum_duration">{{ get_phrase('Minimum duration to watch') }}<span>*</span></label>
                                <div class="input-group">
                                    <input type="text" value="{{ $drip_info->minimum_duration }}" id="minimum_duration"
                                        class="form-control ol-form-control" name="minimum_duration"
                                        data-toggle="timepicker" data-show-meridian="false">
                            ...
                            <div
                                class="fpb-7 mb-2 toggleMinimumWatchField @if ($drip_info->lesson_completion_role == 'duration') d-hidden @endif  ">
                                <label class="form-label ol-form-label"
                                    for="minimum_percentage">{{ get_phrase('Minimum percentage to watch') }}<span
                                        class="required">*</span></label>
                                <div class="input-group">
                                    <input type="text" value="{{ $drip_info->minimum_percentage }}"
                            ...
                            <div class="fpb-7 mb-3">
                                <label class="form-label ol-form-label"
                                    for="locked_lesson_message">{{ get_phrase('Message for locked lesson') }}</label>
