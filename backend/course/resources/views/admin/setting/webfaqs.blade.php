...
<form action="{{ route('admin.website.settings.update') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="col-md-8">
            <div id = "faq_area">
                @php
                    $faqs = get_frontend_json_setting('website_faqs', [['question' => '', 'answer' => '']]);
                @endphp
                        ...
                        <div class="flex-grow-1 px-2 mb-3">
                            <div class="fpb-7 mb-3">
                                ...
                                <input type="text" class="form-control ol-form-control" name="questions[]"
                                    id="questions" placeholder="{{ get_phrase('Write a question') }}"
                                    value="{{ $faq['question'] }}">
                            </div>
                            <div class="fpb-7 mb-3">
                                <label class="form-label ol-form-label">{{ get_phrase('Answer') }}</label>
                                <textarea name="answers[]" class="form-control ol-form-control"
                                    placeholder="{{ get_phrase('Write a question answer') }}">{{ $faq['answer'] }}</textarea>
                            </div>
                        </div>
                        ...
                        @else
                            <div class="pt-4">
                                <button type="button" class="btn ol-btn-light ol-icon-btn mt-2"
                                    data-bs-toggle="tooltip" title="{{ get_phrase('Remove') }}" name="button"
                                    onclick="removeFaq(this)"> <i class="fi-rr-minus-small"></i>
                                </button>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="d-flex pt-2 border-top">
                        <div class="flex-grow-1 px-3">
                            ...
                            <div class="fpb-7 mb-3">
                                <label class="form-label ol-form-label">{{ get_phrase('Answer') }}</label>
                                <textarea name="answers[]" class="form-control ol-form-control"
                                    placeholder="{{ get_phrase('Write a question answer') }}"></textarea>
                            </div>

                        </div>
                        <div class="pt-4">
                            <button type="button" class="btn ol-btn-light ol-icon-btn mt-2"name="button"
                                data-bs-toggle="tooltip" title="{{ get_phrase('Remove') }}" onclick="removeFaq(this)">
