            ...
            <ul class="nav nav-tabs eNav-Tabs-custom eTab" id="myTab" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="cProfile-tab" data-bs-toggle="tab" data-bs-target="#cProfile"
                        type="button" role="tab" aria-controls="cProfile" aria-selected="false">
            ...
            <div class="tab-content eNav-Tabs-content" id="myTabContent">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="form-group">
                                                    <form action="#">
                                                        <input onchange="update_language_dir('{{ $language->id }}', 'ltr')"
                                                            name="direction" id="direction_ltr{{ $language->id }}"
                                                            type="radio" value="ltr"
                                                            @if ($language->direction == 'ltr') checked @endif>
                                                        <label
                                                            for="direction_ltr{{ $language->id }}">{{ get_phrase('LTR') }}</label>
                                                        &nbsp;&nbsp;
                                                        <input onchange="update_language_dir('{{ $language->id }}', 'rtl')"
                                                            name="direction" id="direction_rtl{{ $language->id }}"
                                                            type="radio" value="rtl"
                                                            @if ($language->direction == 'rtl') checked @endif>
                                                        <label
                                                            for="direction_rtl{{ $language->id }}">{{ get_phrase('RTL') }}</label>
                                                    </form>
                                                </div>
                                            </td>
                                            <td class="">
                                                <a href="{{ route('admin.language.phrase.edit', ['lan_id' => $language->id]) }}"
                                                    class="btn btn-light-white">{{ get_phrase('Edit phrase') }}</a>

                                                @if ($language->name == 'english' || $language->name == 'English')
                                                @else
                                                    <a href="{{ route('admin.language.export', ['id' => $language->id]) }}"
                                                        class="btn btn-light-white">{{ get_phrase('Export language') }}</a>
                                                    <a href="javascript:;"
                                                        onclick="confirmModal('{{ route('admin.language.delete', ['id' => $language->id]) }}')"
                                                        class="btn btn-light-white">{{ get_phrase('Delete language') }}</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        <div class="row m-2">
                            <div class="col-md-8">
                                <form action="{{ route('admin.language.store') }}" method="post">
                                    ...
                                    <div class="fpb7 mb-2">
                                        <label for="language"
                                            class="form-label ol-form-label">{{ get_phrase('Add new language') }}</label>
                                        <input type="text" class="form-control ol-form-control" id="language"
                                            name="language"
                                            placeholder="{{ get_phrase('No special character or space is allowed. Valid examples: French, Spanish, Bengali etc') }}">
                ...
                <div class="tab-pane fade" id="cMessage" role="tabpanel" aria-labelledby="cMessage-tab">
                    <div class="tab-pane p-3" id="import_language">
                        <div class="row">
                            <form action="{{ route('admin.language.import') }}" method="post"
                                enctype="multipart/form-data">
                                @csrf
                                <div class="col-md-8 fpb-7 mb-2">
                                    <label for="formFile"
                                        class="form-label ol-form-label">{{ get_phrase('Import your language files from here. (Ex: english.json)') }}</label>
                                    <input class="form-control ol-form-control" type="file" id="formFile"
                                        name="language_file" accept=".json" required>
                                </div>


                                <div class="col-md-8 fpb-7 mb-2">
                                    <label for="language_id"
                                        class="form-label ol-form-label">{{ get_phrase('Select Language to Replace') }}</label>
                                    <select name="language_id" id="language_id" class="form-control ol-form-control"
                                        required>
                                        @foreach ($languages as $language)
                                            @if ($language->name !== 'English')
                                                <!-- Skip default language (English) -->
                                                <option value="{{ $language->id }}">{{ ucfirst($language->name) }}
                                                </option>
                                            @endif
                                        @endforeach
                                    </select>
