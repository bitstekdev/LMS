    ...
    <div class="ol-card radius-8px">
        <div class="ol-card-body my-3 py-12px px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                ...
                <a
                    href="{{ route('admin.course.create') }}"class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px">
                    ...
                    <div class="row mt-3 mb-4">
                        <div class="col-md-6 d-flex align-items-center gap-3">
                                            ...
                                            <div class="filter-option d-flex flex-column gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <select class="form-control ol-form-control ol-select2"
                                                        ...
                                                        <option
                                                            value="active"@if (isset($status) && $status == 'active') selected @endif>
                                                            {{ get_phrase('Active') }} </option>
                                                        <option
                                                            value="inactive"@if (isset($status) && $status == 'inactive') selected @endif>
                                                            {{ get_phrase('Inactive') }} </option>
                                                        <option
                                                            value="pending"@if (isset($status) && $status == 'pending') selected @endif>
                                                            {{ get_phrase('Pending') }} </option>
                                                        <option
                                                            value="upcoming"@if (isset($status) && $status == 'upcoming') selected @endif>
                                                            {{ get_phrase('Upcoming') }} </option>
                                                        <option
                                                            value="private"@if (isset($status) && $status == 'private') selected @endif>
                                                            {{ get_phrase('Private') }} </option>
                                                        <option
                                                            value="draft"@if (isset($status) && $status == 'draft') selected @endif>
                                                            {{ get_phrase('Draft') }} </option>
                                                    </select>
                                                </div>
                            ...
                            @if (isset($_GET) && count($_GET) > 0)
                                <a href="{{ route('admin.courses') }}" class="me-2" data-bs-toggle="tooltip"
                                    ...
                                    <table class="table eTable eTable-2 print-table">
                                        ...
                                        <tbody>
                                            @foreach ($courses as $key => $row)
                                                @php
                                                    $query = App\Models\WatchHistory::where('course_id', $row->id)
                                                    ...
                                                    if (
                                                        isset($query->watching_lesson_id) &&
                                                ...
                                                <tr>
                                                        ...
                                                        <div class="dAdmin_profile d-flex align-items-center min-w-200px">
                                                            <div class="dAdmin_profile_name">
                                                                <h4 class="title fs-14px">
                                                                    <a
                                                                        href="{{ route('admin.course.edit', [$row->id, 'tab' => 'curriculum']) }}">{{ ucfirst($row->title) }}</a>
                                                                </h4>

                                                                <a
                                                                    href="{{ route('admin.courses', ['instructor' => $row->user_id]) }}">
                                                    ...
                                                    <td>
                                                        <div class="sub-title2 text-12px">
                                                            <a
                                                                href="{{ route('admin.courses', ['category' => $row->category->slug ?? '']) }}">{{ category_by_course($row->category_id)->title }}</a>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="sub-title2 text-12px">
                                                            <a
                                                                href="{{ route('admin.course.edit', [$row->id, 'tab' => 'curriculum']) }}">
                                                                <p>{{ get_phrase('Lesson') }}:
                                                                    {{ lesson_count($row->id) }} </p>
                                                                <p> {{ get_phrase('Section') }}:
                                                                    {{ section_count($row->id) }} </p>
                                                            </a>
                                                        </div>
                                                    </td>
                                                    <td>
                                                            ...
                                                            <ul class="dropdown-menu">
                                                                ...
                                                                <li>
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('admin.course.edit', [$row->id, 'tab' => 'basic']) }}">{{ get_phrase('Edit Course') }}</a>
                                                                </li>
                                                                <li>
                                                                    <a class="dropdown-item"
                                                                        onclick="confirmModal('{{ route('admin.course.duplicate', $row->id) }}')"
                                                                        href="javascript:void(0)">{{ get_phrase('Duplicate Course') }}</a>
                                                                </li>
