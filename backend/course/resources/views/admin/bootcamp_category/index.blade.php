...
@section('content')
    <div class="ol-card radius-8px">
        <div class="ol-card-body my-3 py-12px px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                ...
                <a onclick="ajaxModal('{{ route('modal', ['admin.bootcamp_category.create']) }}', '{{ get_phrase('Add new category') }}')"
    ...
    @if ($categories->count() > 0)
                    ...
                    <div class="ol-card card-hover">
                        <div class="ol-card-body px-20px py-3 d-flex justify-content-between">
                            <div>
                                ...
                                <p class="sub-title text-12px mt-2">{{ get_phrase('Total bootcamps') }}
                                    {{ count_bootcamps_by_category($category->id) }}</p>
                            </div>
                            ...
                            <div class="dropdown ol-icon-dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="javascript:void(0);"
                                            onclick="ajaxModal('{{ route('modal', ['admin.bootcamp_category.edit', 'id' => $category->id]) }}', '{{ get_phrase('Edit category') }}')">{{ get_phrase('Edit') }}</a>
                                    </li>
                                    <li><a class="dropdown-item" href="javascript:void(0);"
                                            onclick="confirmModal('{{ route('admin.bootcamp.category.delete', $category->id) }}')">{{ get_phrase('Delete') }}</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
        ...
        <div
            class="admin-tInfo-pagi d-flex justify-content-md-between justify-content-center align-items-center flex-wrap gr-15 mb-5">
            <p class="admin-tInfo">
