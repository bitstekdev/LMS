@foreach (App\Models\PaymentGateway::where('status', 1)->get() as $payment_gateway)
    ...
    @php
        ...
        if (isset($instructor)) {
            $instructor_keys = $instructor->paymentkeys;
        }
    @endphp
    ...
    @if (is_array($gateway_keys))
        @foreach ($gateway_keys as $key => $value)
            @php
                if (
                    isset($instructor_keys) &&
                    is_array($instructor_keys) &&
                    array_key_exists($payment_gateway->identifier, $instructor_keys)
                ) {
            ...
            <div class="row mb-3">
                <label for="{{ $payment_gateway->identifier }}{{ $key }}"
                    class="form-label ol-form-label col-sm-2 col-form-label text-capitalize">{{ get_phrase(str_replace('_', ' ', $key)) }}</label>
                <div class="col-sm-8">
                    <input type="text" name="paymentkeys[{{ $payment_gateway->identifier }}][{{ $key }}]"
                        class="form-control ol-form-control" id="{{ $payment_gateway->identifier }}{{ $key }}"
                        value="{{ $input_val }}">
                </div>
            </div>
        @endforeach
