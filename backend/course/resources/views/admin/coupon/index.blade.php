        ...
        <div class="ol-card-body my-3 py-12px px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                ...
                <a href="javascript:void(0)" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px"
                    onclick="ajaxModal('{{ route('modal', ['admin.coupon.create']) }}', '{{ get_phrase('Add Coupon') }}')">
                                    ...
                                    @foreach ($coupons as $key => $coupon)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="dAdmin_info_name min-w-150px">
                                                    <p><span
                                                            class="badge {{ $coupon->status ? 'bg-success' : 'bg-danger' }} text-white">{{ get_phrase($coupon->status ? 'Active' : 'Inactive') }}</span>
                                                    </p>
                                                </div>
                                            </td>
                                                ...
                                                <div class="dropdown ol-icon-dropdown ol-icon-dropdown-transparent">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        <li><a class="dropdown-item" href="#"
                                                                onclick="confirmModal('{{ route('admin.coupon.status', $coupon->id) }}')">{{ get_phrase($coupon->status ? 'Deactivate' : 'Activate') }}</a>
                                                        </li>
                                                        <li><a class="dropdown-item" href="javascript:void(0);"
                                                                onclick="ajaxModal('{{ route('modal', ['admin.coupon.edit', 'id' => $coupon->id]) }}', '{{ get_phrase('Edit Coupon') }}')">{{ get_phrase('Edit') }}</a>
                                                        </li>
                                                        <li><a class="dropdown-item" href="javascript:void(0);"
                                                                onclick="confirmModal('{{ route('admin.coupon.delete', $coupon->id) }}')">{{ get_phrase('Delete') }}</a>
                                                        </li>
                                                    </ul>
                                                </div>
                                            </td>
