...
<body>
        ...
        <div class="sidebar-body">
            ...
            <div class="card border-0 m-2" id="custom_elem_form">
                <div class="card-body pt-2">
                    ...
                    <form action="#">
                        <div class="mb-3">
                            <label for="certificate_element_content"
                                class="form-label">{{ get_phrase('Enter Text with variable data') }}</label>
                            <textarea name="certificate_element_content" placeholder="{{ get_phrase('Total Lesson') }}:{number_of_lesson}"
                                id="certificate_element_content" rows="3" class="form-control"></textarea>
                        </div>
                        <div class="mb-3">
                            <label for="font_family_auto"
                                class="form-label">{{ get_phrase('Choice a font-family') }}</label><br>
                            <input type="radio" name="font_family" value="auto" id="font_family_auto" checked>
                            <label for="font_family_auto">{{ get_phrase('Auto') }}</label><br>
                            <input type="radio" name="font_family" value="Pinyon Script"
                                id="font_family_pinyon_script"> <label
                                for="font_family_pinyon_script">{{ get_phrase('Pinyon Script') }}</label><br>
                        </div>
                        <div class="mb-3">
                            <label for="font_size" class="form-label">{{ get_phrase('Font Size') }}</label><br>
                            <input type="number" name="font_size" value="16" id="font_size" class="form-control"
                                required>
                        </div>
                        <div class="mb-3">
                            <button type="button" class="btn ol-btn-light-primary w-100"
                                onclick="addElemToCertificate()">{{ get_phrase('Add') }}</button>
                        </div>
                        <div class="mb-5">
                            <button type="button" class="btn ol-btn-primary w-100"
                                onclick="saveTemplate()">{{ get_phrase('Save Template') }}</button>
                        </div>
                    </form>
    ...
    <div id="certificate_builder_content" class="builder dotted-background">
        ...
        @else
            <div id="certificate-layout-module"
                class="certificate-layout-module resizeable-canvas draggable position-relative">
                <img class="certificate-template w-100 h-100"
    ...
    <script>
        ...
        function initialize() {
            ...
            $(".draggable").on("dragend", function(e, pos) {}).on("dragstart", function(e, pos) {}).on("dragstop", function(
                e, pos) {
                positionTracking(this);
            });
        }
