    ...
    <div class="ol-card radius-8px">
        <div class="ol-card-body my-3 py-12px px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                ...
                <a onclick="ajaxModal('{{ route('modal', ['admin.tutor_booking.category_add', 'parent' => 0]) }}', '{{ get_phrase('Add new category') }}')"
                                ...
                                <table class="eTable eTable-2 print-table table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col" class="print-d-none text-center">{{ get_phrase('Action') }}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            <li>
                                                                <a href="#" class="dropdown-item"
                                                                    onclick="ajaxModal('{{ route('modal', ['admin.tutor_booking.category_edit', 'id' => $category->id]) }}', '{{ get_phrase('Edit subject') }}')">
                                                                    {{ get_phrase('Edit') }}</a>
                                                            </li>
                                                            <li>
                                                                @if ($category['status'] == 1)
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('admin.tutor_category_status', ['id' => $category->id, 'status' => 'deactive']) }}">{{ get_phrase('Deactive') }}</a>
                                                                @else
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('admin.tutor_category_status', ['id' => $category->id, 'status' => 'active']) }}">{{ get_phrase('Active') }}</a>
                                                                @endif
                                                            </li>
                                                            <li>
                                                                <a class="dropdown-item"
                                                                    onclick="confirmModal('{{ route('admin.tutor_category_delete', $category->id) }}')">{{ get_phrase('Delete') }}</a>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                </td>
