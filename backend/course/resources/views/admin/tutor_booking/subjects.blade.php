    ...
    <div class="ol-card radius-8px">
        <div class="ol-card-body my-3 py-12px px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                ...
                <a onclick="ajaxModal('{{ route('modal', ['admin.tutor_booking.subject_add']) }}', '{{ get_phrase('Add new subject') }}')"
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            <li>
                                                                <a href="#" class="dropdown-item"
                                                                    onclick="ajaxModal('{{ route('modal', ['admin.tutor_booking.subject_edit', 'id' => $subject->id]) }}', '{{ get_phrase('Edit subject') }}')">
                                                                    {{ get_phrase('Edit') }}</a>
                                                            </li>
                                                            <li>
                                                                @if ($subject['status'] == 1)
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('admin.tutor_subject_status', ['id' => $subject->id, 'status' => 'deactive']) }}">{{ get_phrase('Deactive') }}</a>
                                                                @else
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('admin.tutor_subject_status', ['id' => $subject->id, 'status' => 'active']) }}">{{ get_phrase('Active') }}</a>
                                                                @endif
                                                            </li>
                                                            <li>
                                                                <a class="dropdown-item"
                                                                    onclick="confirmModal('{{ route('admin.tutor_subject_delete', $subject->id) }}')">{{ get_phrase('Delete') }}</a>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                </td>
