...
class WatermarkService
{
    ...
    public static function getWatermarkData(): array
    {
        return [
    ...
    public static function makeTempWatermark(string $logo): string
    {
        ...
        $opacity = get_player_settings('watermark_opacity') ?? 100;
        $position = get_player_settings('watermark_position') ?? 'bottom-right'; // top-left, top, center, etc.
        $offset_x = get_player_settings('watermark_left') ?? 0;
        $offset_y = get_player_settings('watermark_top') ?? 0;
        ...
        $logoImage = $manager->read(public_path($logo))
            ->scale(width: $width, height: $height);

        // Create a transparent canvas to paste onto
        $canvas = $manager->create($width, $height);

        // Place the resized logo with opacity and offsets
        $canvas->place(
            $logoImage,
            $position,
            $offset_x,
    ...
    public static function deleteTempWatermark(string $path): void
    {
