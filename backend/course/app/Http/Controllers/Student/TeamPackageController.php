...
class TeamPackageController extends Controller
{
    ...
    {
        // ✅ Get the TeamPackagePurchase that owns the package with the given slug
        $purchase = TeamPackagePurchase::where('user_id', auth()->id())
            ->where('status', 1)
            ->whereHas('package', function ($query) use ($slug) {
                $query->where('slug', $slug);
            })
            ->with(['package.course']) // eager load
            ->latest()
            ->first();
        ...
        $page_data['course_title'] = $course->title ?? '';
        $page_data['course_slug'] = $course->slug ?? '';

        // ✅ Team members with user info
        $page_data['members'] = TeamPackageMember::with('user')
            ->where('leader_id', auth()->id())
            ->where('team_package_id', $package->id)
            ->paginate(10);

        // ✅ Purchase history with package relationship
        $page_data['purchased_packages'] = TeamPackagePurchase::with('package')
    ...
    public function purchase($id)
    {
        ...
        // Prevent users from buying their own package
        ...
        if (TeamPackagePurchase::where('user_id', auth('web')->id())
            ->where('package_id', $id)
            ->where('status', 1)
            ->exists()) {
        ...
        // Directly create the purchase/enrollment
        TeamPackagePurchase::create([
            'user_id' => auth('web')->id(),
            'package_id' => $package->id,
            'price' => $package->price,
            'payment_method' => 'manual', // or 'free' if needed
            'status' => 1,
            'tax' => 0,
            'admin_revenue' => 0,
            'instructor_revenue' => 0,
            'invoice' => '#'.\Illuminate\Support\Str::random(20),
        ]);

        Session::flash('success', get_phrase('Enrolled in the package successfully.'));

        return redirect()->route('my.team.packages'); // or whatever route lists user's purchased packages
    }
