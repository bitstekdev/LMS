...
class BootcampPurchaseFactory extends Factory
{
    ...
    public function definition(): array
    {
        ...
        return [
            'invoice' => Str::upper(Str::random(3)).'-'.fake()->unique()->numerify('########'),
            'user_id' => $user->id,
            'bootcamp_id' => $bootcamp->id,
            'price' => $price,
            'tax' => $tax,
            'payment_method' => fake()->randomElement(['stripe', 'paypal', 'bank', 'manual']),
            'payment_details' => json_encode(['tx' => Str::uuid()]),
            'status' => fake()->randomElement([0, 1]), // pending/paid
            'admin_revenue' => $admin,
